"""
Backend de acceso a /docs/ para la landing de MedLibra.

No guarda usuarios propios: cada login se valida en tiempo real contra la
instancia real del cliente (POST https://{subdominio}.medlibra.com.ar/auth/verify),
reutilizando el auth que ya existe en cada contenedor (libracore.auth.SessionAuth +
tabla users propia). A diferencia de Contalibra/Restolibra, MedLibra no tiene un
...
patron que ya usan contalibra_web/restolibra_web/gestiolibra_web contra sus
respectivas apps.
"""
import os
import re
...
SECRET_KEY       = os.environ.get("DOCS_SESSION_SECRET", "medlibra-docs-secret-change-me")
APEX_DOMAIN      = os.environ.get("APEX_DOMAIN", "medlibra.com.ar")
...
app = FastAPI(title="MedLibra Docs Auth", docs_url=None, redoc_url=None)
...
<head>
<meta charset="UTF-8">
<title>Acceso a Documentación · MedLibra</title>
...
<style>
  ...
  button {{ width:100%; margin-top:1.25rem; padding:.7rem; background:#0d9488; color:#fff;
            border:none; border-radius:8px; font-size:1rem; cursor:pointer; }}
  button:hover {{ background:#0f766e; }}
  .error {{ background:#fef2f2; color:#b91c1c; padding:.6rem .75rem; border-radius:8px;
            font-size:.85rem; margin-bottom:1rem; }}
  a {{ color:#0d9488; text-decoration:none; font-size:.85rem; }}
</style>
</head>
<body>
  <div class="card">
    ...
    <form method="post" action="/login-docs">
      ...
      <div class="domain-row">
        <input type="text" name="slug" id="slug" required autocomplete="off"
               placeholder="tu-consultorio" value="{slug}">
